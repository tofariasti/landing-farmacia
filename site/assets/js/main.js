(function () {
  ...
  var WHATSAPP_NUMBER = '5551998877666';
  ...
  function initSmoothScroll() {
    document.querySelectorAll('a[href^="#"]').forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        e.preventDefault();
        var header = document.getElementById('header');
        var ticker = document.querySelector('.offers-ticker');
        var offset = (header ? header.offsetHeight : 0) + (ticker ? ticker.offsetHeight : 0);
        window.scrollTo({
  ...
  function initWhatsAppForm() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      var fields = ['nome', 'telefone', 'tipo-pedido', 'descricao', 'retirada'];
      var valid = true;
      ...
      var tipoPedido = document.getElementById('tipo-pedido').value;
      var descricao = document.getElementById('descricao').value;
      var retirada = document.getElementById('retirada').value;
      var observacoes = document.getElementById('observacoes').value;

      var msg = '*Olá! Gostaria de fazer um pedido — Farmácia Vida & Saúde* 💊\n\n';
  ...
  function initWhatsAppFloat() {
    ...
    float.addEventListener('click', function (e) {
      ...
      if (contact && window.scrollY < contact.offsetTop - 200) {
        ...
        var offset = (header ? header.offsetHeight : 0) + (ticker ? ticker.offsetHeight : 0);
        window.scrollTo({
          top: contact.offsetTop - offset - 16,
          behavior: prefersReducedMotion ? 'auto' : 'smooth'
        });
        setTimeout(function () {
  ...
  function validateField(field) {
    ...
    if (group && group.classList.contains('is-hidden')) return true;
